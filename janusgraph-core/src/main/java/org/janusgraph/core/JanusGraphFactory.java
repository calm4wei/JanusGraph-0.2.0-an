...
public class JanusGraphFactory {
     ...
     * @see <a href="http://docs.janusgraph.org/latest/configuration.html">"Configuration" manual chapter</a>
     * @see <a href="http://docs.janusgraph.org/latest/config-ref.html">Configuration Reference</a>
     */
    ...
    public static JanusGraph open(ReadConfiguration configuration, String backupName) {
        final ModifiableConfiguration config = new ModifiableConfiguration(ROOT_NS, (WriteConfiguration) configuration,
        ...
        if (null != graphName) {
            ...
            if (jgm != null) {
                log.warn(
                    "You should supply \"graph.graphname\" in your .properties file configuration if you are opening "
    ...
    public static void drop(JanusGraph graph) throws BackendException {
        ...
        Preconditions.checkArgument(graph instanceof StandardJanusGraph, "Invalid graph instance detected: %s",
            graph.getClass());
        ...
        try {
            ...
            final Pattern p = Pattern.compile(
                "(" + Pattern.quote(STORAGE_NS.getName()) + "\\..*" + "(" + Pattern.quote(STORAGE_DIRECTORY.getName())
                    ...
                    + "\\..*" + "(" + Pattern.quote(INDEX_DIRECTORY.getName()) + "|" + Pattern
                    .quote(INDEX_CONF_FILE.getName()) + ")" + ")");
            ...
            while (keysToMangle.hasNext()) {
                ...
                Preconditions
                    .checkArgument(StringUtils.isNotBlank(s), "Invalid Configuration: key %s has null empty value", k);
