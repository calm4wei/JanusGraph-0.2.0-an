...
public class StandardJanusGraph extends JanusGraphBlueprintsGraph {
    ...
    public  StandardJanusGraph(GraphDatabaseConfiguration configuration) {
        ...
        openTransactions = Collections
            .newSetFromMap(new ConcurrentHashMap<StandardJanusGraphTx, Boolean>(100, 0.75f, 1));
        ...
        if (globalConfig.has(REGISTRATION_TIME, uniqueInstanceId)) {
            throw new JanusGraphException(String.format(
                "A JanusGraph graph with the same instance id [%s] is already open. Might required forced shutdown.",
                uniqueInstanceId));
        }
    ...
    private synchronized void closeInternal() {
        ...
        if (1 == txCloseExceptions.size()) {
            ...
            throw new IllegalStateException(String
                .format("Unable to close %s transactions (see warnings in log output for details)",
    ...
    public RecordIterator<Long> getVertexIDs(final BackendTransaction tx) {
        Preconditions
            .checkArgument(backend.getStoreFeatures().hasOrderedScan() || backend.getStoreFeatures().hasUnorderedScan(),
    ...
    private static final Predicate<InternalRelation> SCHEMA_FILTER = new Predicate<InternalRelation>() {
        ...
        public boolean apply(final InternalRelation internalRelation) {
            return internalRelation.getType() instanceof BaseRelationType && internalRelation
                .getVertex(0) instanceof JanusGraphSchemaVertex;
        }
    };
    ...
    public void commit(final Collection<InternalRelation> addedRelations,
        ...
        try {
            ...
            if (logTransaction) {
                ...
                txLog.add(txLogHeader
                        .serializeModifications(serializer, LogTxStatus.PRECOMMIT, tx, addedRelations, deletedRelations),
                    txLogHeader.getLogKey());
            }

            //3.2 Commit schema elements and their associated relations in a separate transaction if backend does not support
            //    transactional isolation
            boolean hasSchemaElements =
                !Iterables.isEmpty(Iterables.filter(deletedRelations, SCHEMA_FILTER)) || !Iterables
                    .isEmpty(Iterables.filter(addedRelations, SCHEMA_FILTER));
            Preconditions
                .checkArgument(!hasSchemaElements || (!tx.getConfiguration().hasEnabledBatchLoading() && acquireLocks),
                    ...
                    try {
                        ...
                        if (logTxIdentifier != null) {
                            try {
                                ...
                                Future<Message> env = userLog.add(txLogHeader
                                    .serializeModifications(serializer, LogTxStatus.USER_LOG, tx, addedRelations,
                                        deletedRelations));
                                if (env.isDone()) {
                                    try {
                                        env.get();
                        ...
                        if (logTransaction) {
                            ...
                            try {
                                txLog.add(
                                    txLogHeader.serializeSecondary(serializer, status, indexFailures, userlogSuccess),
                                    txLogHeader.getLogKey());
                            } catch (Throwable e) {
                                log.error(
                                    "Could not tx-log secondary persistence status on transaction [" + transactionId
                                        + "]", e);
                            }
                        }
                    }
