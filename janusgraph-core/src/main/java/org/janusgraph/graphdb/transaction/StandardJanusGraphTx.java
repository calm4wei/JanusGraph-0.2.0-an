...
public class StandardJanusGraphTx extends JanusGraphBlueprintsTransaction
    ...
    private class VertexConstructor implements Retriever<Long, InternalVertex> {
        ...
        public InternalVertex get(Long vertexid) {
            ...
            long canonicalVertexId = idInspector.isPartitionedVertex(vertexid) ?
                idManager.getCanonicalVertexId(vertexid) :
                vertexid;
            if (verifyExistence) {
    ...
    public JanusGraphVertex addVertex(Long vertexId, VertexLabel label) {
        ...
        Preconditions
            .checkArgument(vertexId == null || !config.hasVerifyExternalVertexExistence() || !containsVertex(vertexId),
    ...
    private Iterable<InternalVertex> getInternalVertices() {
        ...
        return Iterables.filter(allVertices, new Predicate<InternalVertex>() {
            ...
            public boolean apply(
                ...
                return !isPartitionedVertex(internalVertex) || internalVertex.longId() == idInspector
                    .getCanonicalVertexId(internalVertex.longId());
            }
        });
    }
    ...
    public JanusGraphVertexProperty addProperty(VertexProperty.Cardinality cardi, JanusGraphVertex vertex,
        ...
        if (key.cardinality().convert() != cardi && cardi != VertexProperty.Cardinality.single)
            throw new SchemaViolationException(String
                .format("Key is defined for %s cardinality which conflicts with specified: %s", key.cardinality(),
                    cardi));
        verifyWriteAccess(vertex);
        Preconditions
            .checkArgument(!(key instanceof ImplicitKey), "Cannot create a property of implicit type: %s", key.name());
        ...
        try {
            ...
            if (cardi == VertexProperty.Cardinality.single || cardi == VertexProperty.Cardinality.set) {
                ...
                */
                ...
                if ((!config.hasVerifyUniqueness()
                    || ((InternalRelationType) key).getConsistencyModifier() != ConsistencyModifier.LOCK) && !TypeUtil
                    .hasAnyIndex(key) && cardi == cardinality.convert()) {
            ...
            StandardVertexProperty prop = new StandardVertexProperty(
                IDManager.getTemporaryRelationID(temporaryIds.nextID()), key, (InternalVertex) vertex, normalizedValue,
                ElementLifeCycle.New);
            if (config.hasAssignIDsImmediately())
                graph.assignID(prop);
    ...
    public PropertyKey getPropertyKey(String name) {
        ...
        Preconditions
            .checkArgument(pk == null || pk.isPropertyKey(), "The relation type with name [%s] is not a property key",
                name);
    ...
    public EdgeLabel getEdgeLabel(String name) {
        ...
        Preconditions
            .checkArgument(el == null || el.isEdgeLabel(), "The relation type with name [%s] is not an edge label",
                name);
        return (EdgeLabel) el;
    }
    ...
    public void executeMultiQuery(final Collection<InternalVertex> vertices, final SliceQuery sq,
        ...
        if (!vids.isEmpty()) {
            List<EntryList> results = QueryProfiler
                .profile(profiler, sq, true, q -> graph.edgeMultiQuery(vids, q, txHandle));
            int pos = 0;
            for (JanusGraphVertex v : vertices) {
    ...
    public final QueryExecutor<GraphCentricQuery, JanusGraphElement, JointIndexQuery> elementProcessorImpl = new QueryExecutor<GraphCentricQuery, JanusGraphElement, JointIndexQuery>() {
        ...
        public Iterator<JanusGraphElement> getNew(final GraphCentricQuery query) {
            ...
            if (query.getResultType() == ElementCategory.VERTEX && hasModifications()) {
                ...
                if (standardIndexKey == null) {
                    ...
                    vertices = com.google.common.collect.Iterators.transform(
                        newVertexIndexEntries.get(standardIndexKey.getValue(), standardIndexKey.getKey()).iterator(),
                        new com.google.common.base.Function<JanusGraphVertexProperty, JanusGraphVertex>() {
                            @Nullable
                            @Override
                ...
                return (Iterator) com.google.common.collect.Iterators
                    .filter(vertices, new Predicate<JanusGraphVertex>() {
                        @Override
                        public boolean apply(
                            @Nullable
                ...
                (query.getResultType() == ElementCategory.EDGE || query.getResultType() == ElementCategory.PROPERTY)
                    && !addedRelations.isEmpty()) {
                return (Iterator) addedRelations.getView(new Predicate<InternalRelation>() {
                    @Override
                    public boolean apply(final InternalRelation relation) {
                        return query.getResultType().isInstance(relation) && !relation.isInvisible() && query
                            .matches(relation);
                    }
                }).iterator();
            } else
        ...
        public Iterator<JanusGraphElement> execute(final GraphCentricQuery query, final JointIndexQuery indexQuery,
            ...
            if (!indexQuery.isEmpty()) {
                ...
                iter = new SubqueryIterator(indexQuery.getQuery(0), indexSerializer, txHandle, indexCache,
                    indexQuery.getLimit(), getConversionFunction(query.getResultType()), retrievals.isEmpty() ?
                    null :
                ...
                if (config.hasForceIndexUsage())
                    throw new JanusGraphException(
                        "Could not find a suitable index to answer graph query and graph scans are disabled: " + query);
