...
public class SolrIndex implements IndexProvider {
    ...
    public static final ConfigOption<Integer> MAX_SHARDS_PER_NODE = new ConfigOption<Integer>(SOLR_NS,
        "max-shards-per-node",
    ...
    public static final ConfigOption<Integer> REPLICATION_FACTOR = new ConfigOption<Integer>(SOLR_NS,
        "replication-factor",
    ...
    private static final IndexFeatures SOLR_FEATURES = new IndexFeatures.Builder().supportsDocumentTTL()
        ...
        .supportsCardinality(Cardinality.SET).supportsCustomAnalyzer().supportsGeoContains().build();
    ...
    public SolrIndex(final Configuration config) throws BackendException {
        ...
        if (mode == Mode.CLOUD) {
            ...
            final CloudSolrClient cloudServer = new CloudSolrClient.Builder().withZkHost(zookeeperUrl)
            ...
            solrClient = new LBHttpSolrClient.Builder().withHttpClient(clientParams)
    ...
    public void register(String store, String key, KeyInformation information, BaseTransaction tx)
        ...
        if (analyzer != null) {
            ...
            try {
                ((Constructor<Tokenizer>) ClassLoader.getSystemClassLoader().loadClass(analyzer).getConstructor())
                    .newInstance();
            ...
            for (final Map.Entry<String, Map<String, IndexMutation>> stores : mutations.entrySet()) {
                ...
                for (final Map.Entry<String, IndexMutation> entry : stores.getValue().entrySet()) {
                    ...
                    if (mutation.hasAdditions()) {
                        ...
                        if (ttl > 0) {
                            Preconditions
                                .checkArgument(isNewDoc, "Solr only supports TTL on new documents [%s]", docId);
    ...
    private void handleRemovalsFromIndex(String collectionName, String keyIdField, String docId,
        List<IndexEntry> fieldDeletions, KeyInformation.IndexRetriever informations)
    ...
    private Map<String, Object> collectFieldValues(List<IndexEntry> content, String collectionName,
        KeyInformation.IndexRetriever informations) throws BackendException {
    ...
    @Override
    public Stream<String> query(IndexQuery query, KeyInformation.IndexRetriever informations, BaseTransaction tx)
        throws BackendException {
        ...
        if (!query.getOrder().isEmpty()) {
            ...
            for (final IndexQuery.OrderEntry order1 : orders) {
                ...
                final SolrQuery.ORDER order =
                    order1.getOrder() == Order.ASC ? SolrQuery.ORDER.asc : SolrQuery.ORDER.desc;
    ...
    private SolrQuery runCommonQuery(RawQuery query, KeyInformation.IndexRetriever informations, BaseTransaction tx,
    ...
    @Override
    public Stream<RawQuery.Result<String>> query(RawQuery query, KeyInformation.IndexRetriever informations,
        BaseTransaction tx) throws BackendException {
        final String collection = query.getStore();
        final String keyIdField = getKeyFieldId(collection);
        return executeQuery(query.hasLimit() ? query.getLimit() : null, query.getOffset(), collection,
            runCommonQuery(query, informations, tx, collection, keyIdField), doc -> {
    ...
    @Override
    public Long totals(RawQuery query, KeyInformation.IndexRetriever informations, BaseTransaction tx)
        throws BackendException {
        try {
            final String collection = query.getStore();
            final String keyIdField = getKeyFieldId(collection);
            final QueryResponse response = solrClient
                .query(collection, runCommonQuery(query, informations, tx, collection, keyIdField));
    ...
    public String buildQueryFilter(Condition<JanusGraphElement> condition, KeyInformation.StoreRetriever informations) {
        if (condition instanceof PredicateCondition) {
            ...
            if (value instanceof Number) {
                ...
                Preconditions.checkArgument(janusgraphPredicate instanceof Cmp,
                    "Relation not supported on numeric types: " + janusgraphPredicate);
                ...
                if (map == Mapping.TEXT && !Text.HAS_CONTAINS.contains(janusgraphPredicate))
                    throw new IllegalArgumentException(
                        "Text mapped string values only support CONTAINS queries and not: " + janusgraphPredicate);
                if (map == Mapping.STRING && Text.HAS_CONTAINS.contains(janusgraphPredicate))
                    throw new IllegalArgumentException(
                        "String mapped string values do not support CONTAINS queries: " + janusgraphPredicate);

                //Special case
                if (janusgraphPredicate == Text.CONTAINS) {
                    return tokenize(informations, value, key, janusgraphPredicate, (String) ParameterType.TEXT_ANALYZER
                        .findParameter(informations.get(key).getParameters(), null));
                    ...
                    final String tokenizer = (String) ParameterType.STRING_ANALYZER
                        .findParameter(informations.get(key).getParameters(), null);
                    if (tokenizer != null) {
                    ...
                    throw new IllegalArgumentException(
                        "Relation is not supported for string value: " + janusgraphPredicate);
                }
            } else if (value instanceof Geoshape) {
                final Mapping map = Mapping.getMapping(informations.get(key));
                Preconditions.checkArgument(janusgraphPredicate instanceof Geo && janusgraphPredicate != Geo.DISJOINT,
                    "Relation not supported on geo types: " + janusgraphPredicate);
                Preconditions.checkArgument(map == Mapping.PREFIX_TREE || janusgraphPredicate == Geo.WITHIN
                        || janusgraphPredicate == Geo.INTERSECT,
                    "Relation not supported on geopoint types: " + janusgraphPredicate);
                final Geoshape geo = (Geoshape) value;
                if (geo.getType() == Geoshape.Type.CIRCLE && (janusgraphPredicate == Geo.INTERSECT
                    || map == Mapping.DEFAULT)) {
                ...
                } else if (geo.getType() == Geoshape.Type.BOX && (janusgraphPredicate == Geo.INTERSECT
                    || map == Mapping.DEFAULT)) {
                    final Geoshape.Point southwest = geo.getPoint(0);
                    final Geoshape.Point northeast = geo.getPoint(1);
                    return (key + ":[" + southwest.getLatitude() + "," + southwest.getLongitude() + " TO " + northeast
                ...
                final String queryValue = escapeValue(
                    value instanceof Date ? toIsoDate((Date) value) : value.toString());
                Preconditions.checkArgument(janusgraphPredicate instanceof Cmp,
                    "Relation not supported on date types: " + janusgraphPredicate);
                final Cmp numRel = (Cmp) janusgraphPredicate;
                ...
                if (janusgraphPredicate == Cmp.EQUAL) {
                    ...
                    throw new IllegalArgumentException(
                        "Relation is not supported for uuid value: " + janusgraphPredicate);
                }
            } else
    ...
    public boolean supports(KeyInformation information, JanusGraphPredicate janusgraphPredicate) {
        ...
        if (mapping != Mapping.DEFAULT && !AttributeUtil.isString(dataType) && !(mapping == Mapping.PREFIX_TREE
            && AttributeUtil.isGeo(dataType)))
            return false;
        ...
        if (Number.class.isAssignableFrom(dataType)) {
            ...
            case PREFIX_TREE:
                return janusgraphPredicate == Geo.INTERSECT || janusgraphPredicate == Geo.WITHIN
                    || janusgraphPredicate == Geo.CONTAINS;
            }
    ...
    public boolean supports(KeyInformation information) {
        ...
        if (Number.class.isAssignableFrom(dataType) || dataType == Date.class || dataType == Instant.class
            ...
            if (mapping == Mapping.DEFAULT || mapping == Mapping.PREFIX_TREE)
                return true;
        }
        return false;
    }
    ...
    public String mapKey2Field(String key, KeyInformation keyInfo) {
        Preconditions
            .checkArgument(!StringUtils.containsAny(key, new char[] { ' ' }), "Invalid key name provided: %s", key);
        if (!dynFields)
            return key;
    ...
    private static Map<Geo, String> spatialPredicates() {
        return Collections.unmodifiableMap(Stream
            .of(new SimpleEntry<>(Geo.WITHIN, "IsWithin"), new SimpleEntry<>(Geo.CONTAINS, "Contains"),
                new SimpleEntry<>(Geo.INTERSECT, "Intersects"), new SimpleEntry<>(Geo.DISJOINT, "IsDisjointTo"))
    ...
    private static void createCollectionIfNotExists(CloudSolrClient client, Configuration config, String collection)
        ...
        if (!checkIfCollectionExists(client, collection)) {
            ...
            final String genericConfigSet = config.has(SOLR_DEFAULT_CONFIG) ?
                config.get(SOLR_DEFAULT_CONFIG) :
                collection;

            final CollectionAdminRequest.Create createRequest = CollectionAdminRequest
